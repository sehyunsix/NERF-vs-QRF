...
\quad 양자 머신러닝은 다양한 양자 알고리즘 연구 분야 중 NISQ(Noisy Intermediate-Scale Quantum) 시대에 그 가능성을 인정받아 최근 가장 활발히 연구가 진행되고 있는 주제이다. 주된 목적으로는, 양자 역학의 여러 원리를 활용하는 양자컴퓨터를 통해 기존의 머신러닝으로 이루어진 시도보다 더 우월한 성능을 내거나, 혹은 비용을 줄이는 등이다. 이를 수학적 이론을 통해 입증하고, 다양한 문제에 적용하는 것 또한 양자머신러닝이 가진 숙제이다. 이렇듯 양자머신러닝은 다양한 분야로의 적용 가능성과 함께 방법론도 활발히 연구되고 있는데, 본 연구에서는 양자 머신러닝의 근간이 되는 양자 알고리즘의 수학적 원리와 양자 머신러닝의 두 가지 대표적인 방법론, 그리고 본질적으로 선형적인 양자컴퓨팅의 연산 방식에 비선형성을 도입하는 방법과 확장 가능성에 대해 논의한다.
...
\input{chaps/ch2-figs} % ch-theo : Figure and Table tex 파일인 듯, 추후 수정 요망
...
\input{chaps/ch3-ResearchFlow} % 3장 tex 파일
...
\input{chaps/ch4-Research} % 4장 tex 파일

% ------------ Chapter 5 시작 ------------ %
\chapter{Conclusion}\label{c:conclusion}
\input{chaps/ch5-Conclusion} % 5장 tex 파일
