...
\begin{pf} 
    ...
    이 회로는 먼저 \(s\) 레지스터의 모든 qubit에 Hadamard 연산을 작동시켜 모든 basis state이 동일한 확률로 중첩된 state \(\displaystyle \sum_{k=0}^{P-1} {1\over {2^{p/2}}}\ket{k}\)를 생성한다. 이렇게 생성된 state의 각 basis state는 \(C_{s}^{ctrl:k}U_{z_k}(\vec{x}, \vec{w}_k, b_k)\)에 의해 basis의 이진 표현에 해당하는 $\ket{\psi_{z_k}}$와 결부되어, 결과적으로는 다음과 같은 state를 만들게 된다:
    \[
