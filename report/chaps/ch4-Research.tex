...
$V_m$의 동작을 살펴보자. \(V_m = C_{a_m}U_{\mathbf{z}}(\vec{x}, W, \vec{b})_q C_{a_m}X^{\otimes n}_q C_q^nH_{a_m}\)으로, 세 개의 step으로 이루어진다.
...
3. \(C_{a_m}U_{\mathbf{z}}(\vec{x}, W, \vec{b})_q\), \(a_m\)의 state가 \(\ket 1\)인 경우에 \(q\) 레지스터에 대해 \(U_{\mathbf{z}}(\vec{x}, W, \vec{b})_q\)를 적용한다.
